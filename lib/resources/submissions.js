...
module.exports = (service, endpoint) => {
  ...
  // TODO: a lot of overlap with POST /submission and POST …/submissions
  // but this one is sort of more complicated internally since it has to handle the overwrite case.
  service.put('/projects/:projectId/forms/:formId/submissions/:instanceId.xml', endpoint(({ Audit, Project, Submission }, { params, body, auth }) =>
    Project.getById(params.projectId)
      .then(getOrNotFound)
      .then((project) => auth.canOrReject('submission.read', project)
        .then(() => Promise.all([
          Submission.fromXml(body),
          project.getFormByXmlFormId(params.formId).then(getOrNotFound)
        ])
        .then(([ partial, form ]) => ((partial.xmlFormId !== params.formId)
          ? reject(Problem.user.unexpectedValue({ field: 'form id', value: partial.xmlFormId, reason: 'did not match the form ID in the URL' }))
          : ((partial.instanceId !== params.instanceId)
            ? reject(Problem.user.unexpectedValue({ field: 'instance id', value: partial.instanceId, reason: 'did not match the instance ID in the URL' }))
            : (!form.acceptsSubmissions()
              ? reject(Problem.user.notAcceptingSubmissions())
              : Submission.getById(form.id, partial.instanceId))))
          .then((extant) => auth.canOrReject(extant.isDefined() ? 'submission.update' : 'submission.create', project)
            .then(() => partial.complete(form, auth.actor()))
            .then((incoming) => (extant.isDefined())
              ? extant.get().with(incoming).update()
              : incoming.create())
            .then((submission) => Promise.all([
              submission.createExpectedAttachments(form),
              Audit.log(auth.actor(), 'submission.create', form, { submissionId: submission.id })
